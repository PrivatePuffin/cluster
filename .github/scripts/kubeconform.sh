...
# echo "=== Validating standalone manifests in ${KUBERNETES_DIR}/main ==="
# find "${KUBERNETES_DIR}/main" -maxdepth 1 -type f -name '*.yaml' -print0 | while IFS= read -r -d $'\0' file;
#   do
...
# done
#
# echo "=== Validating kustomizations in ${KUBERNETES_DIR}/main ==="
# find "${KUBERNETES_DIR}/main" -type f -name $kustomize_config -print0 | while IFS= read -r -d $'\0' file;
#   do
...
echo "=== Validating kustomizations in ${KUBERNETES_DIR}/core ==="
find "${KUBERNETES_DIR}/core" -type f -name $kustomize_config -print0 | while IFS= read -r -d $'\0' file;
  do
    echo "=== Validating kustomizations in ${file/%$kustomize_config} ==="
    kustomize build "${file/%$kustomize_config}" "${kustomize_args[@]}" | \
      kubeconform "${kubeconform_args[@]}"
